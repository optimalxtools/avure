...
const CAPTURE_SECTIONS: CaptureSection[] = [
  {
    title: "Shipment builder",
    description:
      "Plan outbound shipments that will later update distribution performance dashboards.",
    fields: [
      {
        id: "shipment-date",
        label: "Shipment date",
        placeholder: "When should the truck depart?",
        type: "date",
      },
      {
        id: "shipment-destination",
        label: "Destination",
        placeholder: "Customer or depot receiving the load",
      },
      {
        id: "shipment-order",
        label: "Order reference",
        placeholder: "Select the sales order fulfilled",
        type: "select",
        options: ["Order placeholder", "Export consignment"],
      },
      {
        id: "shipment-pallets",
        label: "Pallets",
        placeholder: "Total pallets or bins scheduled",
        type: "number",
      },
      {
        id: "shipment-instructions",
        label: "Special instructions",
        placeholder: "Temperature, documents, loading requirements",
        type: "textarea",
      },
    ],
    actionLabel: "Plan shipment (placeholder)",
  },
  {
    title: "Logistics check-ins",
    description: "Capture en-route updates that keep the distribution team aligned.",
    fields: [
      {
        id: "checkin-route",
        label: "Route",
        placeholder: "Route or lane name",
      },
      {
        id: "checkin-status",
        label: "Status",
        placeholder: "In transit, delayed, delivered, etc.",
        type: "select",
        options: ["Scheduled", "In transit", "Delayed", "Delivered"],
      },
      {
        id: "checkin-eta",
        label: "Updated ETA",
        placeholder: "Provide expected arrival",
      },
      {
        id: "checkin-notes",
        label: "Notes",
        placeholder: "Context for delays or follow-up",
        type: "textarea",
      },
    ],
  },
  {
    ...
    description:
      "Outline the after-delivery data points we want to bring into service reporting.",
    fields: [
      {
      ...
      },
      {
        ...
        placeholder: "Call, email, survey, etc.",
        type: "select",
        options: ["Phone", "Email", "Portal", "Other"],
      },
      {
        id: "feedback-sentiment",
        label: "Sentiment",
        placeholder: "Positive, neutral, follow-up required",
        type: "select",
        options: ["Positive", "Neutral", "Follow-up required"],
      },
      {
        id: "feedback-summary",
        label: "Summary",
        placeholder: "Key points to reflect in service KPIs",
        type: "textarea",
      },
    ],
  },
...
const CAPTURE_HELPER_CARD = (
  <Card className="border-dashed border-muted/50 bg-muted/20">
    <CardHeader>
      <CardTitle>Distribution capture planning</CardTitle>
      <CardDescription>
        Validate the information logistics will capture so downstream fulfilment metrics are ready
        when we switch it on.
      </CardDescription>
    </CardHeader>
    <CardContent>
      <p className="text-sm text-muted-foreground">
        These blocks mirror the hand-offs between packhouse dispatch, logistics tracking and
        customer success while we finalise integrations.
      </p>
    </CardContent>
  </Card>
)
...
export default function Page() {
  ...
  resetSeason,
    ...
    pageKey: "filters:distribution:data-acquisition",
