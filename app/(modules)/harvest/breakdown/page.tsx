...
export default function Page() {
  ...
  resetSeason,
    ...
    pageKey: "filters:harvest:breakdown",
    ...
    <>
      <header className="flex h-16 shrink-0 items-center gap-2 transition-[width,height] ease-linear group-has-[[data-collapsible=icon]]/sidebar-wrapper:h-12">
        <div className="flex items-center justify-between w-full px-4">
          ...
          <div className="flex items-center gap-2">
            <AIButton currentPage="Harvest - Breakdown" />
          </div>
        </div>
      </header>
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center">
            <div className="order-2 md:order-1 grid w-full gap-3 grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 md:flex-1">
              ...
              <div className={FILTER_WRAPPER_CLASS}>
                <span className="absolute -top-2 left-2 bg-background px-1 text-xs text-muted-foreground">Variety</span>
                <Select value={variety} onValueChange={setVariety}>
                  <SelectTrigger
                    className={`w-full h-9 text-sm ${lockVariety ? LOCKED_SELECT_TRIGGER_CLASS : ""} ${varietyDisabled && !lockVariety ? "opacity-60 pointer-events-none" : ""}`}
                    aria-label="Variety"
              ...
              {/* PUC */}
              <div className={FILTER_WRAPPER_CLASS}>
                <span className="absolute -top-2 left-2 bg-background px-1 text-xs text-muted-foreground">PUC</span>
                <Select value={puc} onValueChange={setPuc}>
                  <SelectTrigger
                    className={`w-full h-9 text-sm ${lockPuc ? LOCKED_SELECT_TRIGGER_CLASS : ""} ${pucDisabled && !lockPuc ? "opacity-60 pointer-events-none" : ""}`}
                    aria-label="PUC"
              ...
              {/* Block */}
              <div className={FILTER_WRAPPER_CLASS}>
                <span className="absolute -top-2 left-2 bg-background px-1 text-xs text-muted-foreground">Block</span>
                <Select value={block} onValueChange={setBlock}>
                  <SelectTrigger
                    className={`w-full h-9 text-sm ${lockBlock ? LOCKED_SELECT_TRIGGER_CLASS : ""} ${blockDisabled && !lockBlock ? "opacity-60 pointer-events-none" : ""}`}
                    aria-label="Block"
              ...
              {/* Season */}
              <div className={FILTER_WRAPPER_CLASS}>
                <span className="absolute -top-2 left-2 bg-background px-1 text-xs text-muted-foreground">Season</span>
                <Select value={season} onValueChange={setSeason}>
                  <SelectTrigger
                    className={`w-full h-9 text-sm ${lockSeason ? LOCKED_SELECT_TRIGGER_CLASS : ""} ${seasonDisabled && !lockSeason ? "opacity-60 pointer-events-none" : ""}`}
                    aria-label="Season"
